...
function Books() {
  ...
  const handleExportToExcel = () => {
    const exportData = filteredBooks.map((book, index) => ({
      No: (currentPage - 1) * ITEMS_PER_PAGE + index + 1, // Tartib raqamlarini qo'shish
      title: book.title,
      language: book.language,
      letter: book.letter,
      isScanned: book.isScanned ? "Yes" : "No",
    }));
        ...
        <TableBody>
          {paginatedBooks.map((book, index) => (
            <TableRow key={index}>
              ...
              <TableCell>{book.isScanned ? "Yes" : "No"}</TableCell>
              <TableCell>{book.language}</TableCell>
              <TableCell>{book.letter}</TableCell>
            </TableRow>
          ))}
        </TableBody>
      </Table>
